...
class LocalStorageBackend(SQLiteStorageMixin, StorageBackend):
    ...
    def save_txt_snapshot(self, data: NewsData) -> Optional[str]:
        """
        保存 Markdown 快照 (原本為 TXT 快照，已升級為 Markdown 格式)

        新结构：output/md/{date}/{time}.md
        ...
        try:
            ...
            md_dir = self.data_dir / "md" / date_folder
            md_dir.mkdir(parents=True, exist_ok=True)

            file_path = md_dir / f"{data.crawl_time}.md"

            with open(file_path, "w", encoding="utf-8") as f:
                f.write(f"# 熱榜快照 - {date_folder} {data.crawl_time}\n\n")
                ...
                for source_id, news_list in data.items.items():
                    ...
                    if source_name and source_name != source_id:
                        f.write(f"## {source_name} ({source_id})\n\n")
                    else:
                        f.write(f"## {source_id}\n\n")
                    ...
                    for item in sorted_news:
                        ...
                        if item.url:
                            line = f"{item.rank}. [{title_escaped}]({item.url})"
                        else:
                            line = f"{item.rank}. {title_escaped}"
                            
                        if item.mobile_url:
                            line += f" ([手機版]({item.mobile_url}))"
            ...
            print(f"[本地存储] Markdown 快照已保存: {file_path}")
            return str(file_path)

        except Exception as e:
            print(f"[本地存储] 保存 Markdown 快照失败: {e}")
            return None
    ...
    def cleanup_old_data(self, retention_days: int) -> int:
        ...
        try:
            ...
            # 清理快照目录 (txt/, md/, html/)
            for snapshot_type in ["txt", "md", "html"]:
